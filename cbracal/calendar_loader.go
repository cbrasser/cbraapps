...
var ErrNotACalendar = fmt.Errorf("resource is not a calendar")
	...
	for _, url := range urlsToTry {
		...
		if resp.StatusCode == http.StatusOK {
			...
			if strings.HasPrefix(trimmedBody, "BEGIN:VCALENDAR") {
				...
				// Not a calendar format we recognize
				return nil, ErrNotACalendar
			}
...
func loadAllCalendars(radicaleConfig *RadicaleConfig) ([]Event, map[string]lipgloss.Color, map[string]string, error) {
	...
	if configErr == nil && config != nil {
		...
		if len(config.LocalCalendars) > 0 {
			...
			var baseDir string
			localConfigs := []string{"config.yaml", "config.yml", "calendars.yaml", "calendars.yml"}
			foundLocal := false
			...
			if !foundLocal {
				// Build mode: use data directory for local .ics files
